...
layout (location = 6) in float fr;
layout (location = 7) in float fg;
layout (location = 8) in float fb;
...
vec3 specularContribution(vec3 L, vec3 V, vec3 N, vec3 F0, float metallic, float roughness, vec4 LColor)
{
...
void main() {
	...
	for(int i = 0; i < ubo.pointLights.length(); i++) {
		...
		Lo += specularContribution(L, V, N, F0, fragmetallic, fragroughness, ubo.pointLights[i].color);
	}

...
namespace jhb {
	...
	void HelloTriangleApplication::create2DModelForBRDFLUT()
	{
		std::unique_ptr<jhb::Model::Builder> builder = std::make_unique<jhb::Model::Builder>(device);
		std::vector<std::string> cubefiles = { "Texture/pisa_cube.ktx" };
		builder->loadTextrue3D(cubefiles);
	...
	void HelloTriangleApplication::generateIrradianceCube(std::vector<VkDescriptorSetLayout> desclayouts, std::vector<VkDescriptorSet> descSets)
	{
	...
	void HelloTriangleApplication::generatePrefilteredCube(std::vector<VkDescriptorSetLayout> desclayouts, std::vector<VkDescriptorSet> descSets)
	{
